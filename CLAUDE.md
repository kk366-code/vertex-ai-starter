...
UI（テンプレート・ボタン文言・ヘルプテキスト等）、コミットメッセージ、PRタイトル・本文、コード内コメント・識別子に、具体的な会社名・サービス名・製品名（例: Kindle, Amazon, iPhone, Twitter 等）を含めないこと。代わりに「電子書籍リーダーのスクリーンショット」「スマートフォン端末」のような一般的な表現を使う。

- **Why**: 商標・ブランド名を不必要に露出させない。特定サービスへの依存を強調せず、汎用的なプロダクトとして見せるため。
- **How to apply**: UI 文言を書く前と、PR・コミットメッセージを書く前に、固有名詞が紛れ込んでいないか確認する。クラス名・関数名・スキーマ名にも適用する（ただし `GOOGLE_CLOUD_PROJECT` のような技術スタック名や環境変数は除外）。
